...
// Enter maximum character input (20 characters)
String maxInput = '12345678900987654321'
WebUI.setText(findTestObject('Page_e-Konek Apps - SF Status Uploader/input_HAWB'), maxInput)

// Verify the field actually accepted all 20 characters
String actualValue = WebUI.getAttribute(findTestObject('Page_e-Konek Apps - SF Status Uploader/input_HAWB'), 'value')
assert actualValue.length() == 20, "Field did not accept 20 characters — only accepted ${actualValue.length()} character(s)."
println "Field accepted the full 20-character input: '${actualValue}'"
...
// Verify system responded correctly with either results or no records message
...
if (hasResults) {
    println "System accepted the max-character input and displayed ${rows.size()} matching row(s)."
} else if (hasNoRecordsMessage) {
    println "System accepted the max-character input and correctly showed 'no records to display'."
} else {
    WebUI.closeBrowser()
