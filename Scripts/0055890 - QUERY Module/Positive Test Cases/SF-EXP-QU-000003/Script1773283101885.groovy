...
// Verify that all data columns are visible
List<TestObject> columns = [
    findTestObject('Page_e-Konek Apps - SF Status Uploader/div_HAWB COLUMN'),
    findTestObject('Page_e-Konek Apps - SF Status Uploader/div_SUBMISSION STATUS'),
    findTestObject('Page_e-Konek Apps - SF Status Uploader/div_EVENT CODE'),
    findTestObject('Page_e-Konek Apps - SF Status Uploader/div_REASON CODE'),
    findTestObject('Page_e-Konek Apps - SF Status Uploader/div_REASON NAME'),
    findTestObject('Page_e-Konek Apps - SF Status Uploader/div_STATUS DATE'),
    findTestObject('Page_e-Konek Apps - SF Status Uploader/div_SF TRANSMISSION DATE'),
    findTestObject('Page_e-Konek Apps - SF Status Uploader/div_UPLOADED DATE'),
    findTestObject('Page_e-Konek Apps - SF Status Uploader/div_TYPE')
]

// Check each column is visible
List<String> columnNames = [
    'HAWB', 'SUBMISSION STATUS', 'EVENT CODE', 'REASON CODE', 'REASON NAME',
    'STATUS DATE', 'SF TRANSMISSION DATE', 'UPLOADED DATE', 'TYPE'
]

for (int i = 0; i < columns.size(); i++) {
    boolean isVisible = WebUI.verifyElementVisible(columns[i], FailureHandling.CONTINUE_ON_FAILURE)
    if (isVisible) {
        println "Column visible: ${columnNames[i]}"
    } else {
        println "Column MISSING: ${columnNames[i]}"
    }
}
