...
// Grab all Submission Status values from the table
WebDriver driver = DriverFactory.getWebDriver()
List<String> statuses = driver.findElements(By.xpath("//div[@id='cell-2-undefined']"))
                              .collect { it.getText().trim() }
                              .findAll { it != '' }

println "Submission Statuses found: ${statuses}"

// Verify each row has a valid status
List<String> invalidRows = []
for (int i = 0; i < statuses.size(); i++) {
    // Accept Y, N, or E****
    boolean isValid = statuses[i] == 'Y' || statuses[i] == 'N' || statuses[i].startsWith('E')
    if (!isValid) {
        invalidRows.add("Row ${i + 1}: '${statuses[i]}'")
        println "Invalid status at row ${i + 1}: '${statuses[i]}'"
    } else {
        println "Valid status at row ${i + 1}: '${statuses[i]}'"
    }
}

// Fail the test if any invalid values were found
if (invalidRows.size() > 0) {
    WebUI.closeBrowser()
    assert false, "Invalid Submission Status values found:\n${invalidRows.join('\n')}"
}
