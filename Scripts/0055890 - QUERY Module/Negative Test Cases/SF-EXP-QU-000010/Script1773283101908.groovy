...
// Enter special characters input
...
// Verify system shows no results for special character input
...
boolean hasResults = driver.findElements(By.xpath(
    "//div[@id='cell-1-undefined']")).findAll { it.getText().trim() != '' }.size() > 0

if (hasNoRecordsMessage && !hasResults) {
    println "PASS: No records found for special character input."
} else if (hasResults) {
    WebUI.closeBrowser()
    assert false, "FAIL: Results returned for special character input."
} else {
    WebUI.closeBrowser()
